    ...
    : control_cycle_msec_(0)
{
    ...
    result["grip"] = new DynamixelState();
...
void DemoModule::Initialize(const int control_cycle_msec)
{
...
//    Eigen::Quaterniond _quaternion = rotation2quaternion( ManipulatorH->manipulator_link_data[ _end_index ]->orientation );
    ...
    Eigen::Quaterniond _quaternion = rotation2quaternion( Robotis->curr_orientation );
...
void DemoModule::GripTraGeneProc()
{
    ...
    double _tar_value = grip_val * deg2rad;
...
void DemoModule::Process(std::map<std::string, Dynamixel *> dxls, std::map<std::string, double> sensors)
{
    if(enable == false)
    {
...
//        ROS_INFO("%s : %f", _joint_name.c_str() , result[_joint_name]->goal_position );
    }
